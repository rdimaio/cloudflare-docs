...
// Require the client to send Content-Type: image/png
...
- **Signing `Content-Type`**: Specify the expected `Content-Type` in your SDK's parameters. The signature will include this header, so uploads will fail with a `403/SignatureDoesNotMatch` error if the client sends a different value. This verifies only the declared header. It does not inspect the uploaded bytes or verify the file type.
- **Validating uploaded objects**: Keep untrusted objects private. Validate the object's size and inspect its contents before making it available or processing it. For additional recommendations, refer to the [OWASP File Upload Cheat Sheet](https://cheatsheetseries.owasp.org/cheatsheets/File_Upload_Cheat_Sheet.html).
- **Configuring CORS**: If your presigned URLs will be used from a browser, set up [CORS rules](/r2/buckets/cors/#use-cors-with-a-presigned-url) on your bucket to control which browser origins can make requests.
...
A signed `Content-Type` verifies only that the request contains the expected header value. It does not validate the uploaded object's contents. Keep untrusted objects private until you validate their size and inspect their contents.
